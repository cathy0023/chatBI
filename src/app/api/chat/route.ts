...
export async function POST(request: NextRequest) {
  try {
    ...
    return createSSEStream(async (send) => {
      ...
      // Inline post-process: rewrite month-comparison SQL if LLM grouped by name/dept
      // Skip UNION queries — they're intentionally multi-part (individual vs overall)
      if (/对比|比较|相比/.test(message) && !/\bUNION\b/i.test(result.sql)) {
        ...
        if (groupByMatch && /\bname\b|\bdepartment\b/i.test(groupByMatch[1])) {
          ...
          } catch { /* keep original result if rewrite fails */ }
        }
      }

      // No results — try fuzzy name matching before giving up
      if (result.records.length === 0) {
        const suggestion = suggestSimilarName(db, result.sql, message);
        if (suggestion) {
          send('text', { text: suggestion });
          persistMessage(sid, 'assistant', suggestion);
          send('done', {});
          return;
        }
      ...
      // UNION query with missing individual data — detect and suggest
      ...
      // Pre-compute aggregated data (same logic as chart, so numbers always match)
      const { dimension, metric } = recommendChart(message, result.records);

      // Detect multi-series comparison data (UNION ALL with multiple name values)
      ...
      if (isComparison) {
        // Multi-series: group by name for each month
